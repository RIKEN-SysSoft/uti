...
static uti_hwloc_topology_t topo;
static int max_cpu_os_index = -1;

static int shm_fd;
static char shm_fn[] = "/uti";
static int shm_leader;
static int *nthr_per_pu; /* #threads histogram indexed by uti_hwloc_obj::os_index */
...
static void cpuset_snprintf(char *dst, size_t size, uti_hwloc_cpuset_t cpuset)
{
		cpu_set_t schedset;
		uti_hwloc_cpuset_to_glibc_sched_affinity(topo, cpuset, &schedset, sizeof(schedset));
		schedset_snprintf(dst, size, &schedset);
}

static void pr_cpuset(char *msg, uti_hwloc_cpuset_t cpuset)
{
	char *buf;
	if (!(buf = schedset_alloc())) {
		pr_err("pr_debug_cpuset: error: allocating buf\n");
...
uti_hwloc_obj_t get_cache_obj_by_pu(uti_hwloc_obj_type_t type, uti_hwloc_obj_t cpu)
{
	uti_hwloc_obj_t obj = NULL;
	while ((obj = uti_hwloc_get_next_obj_by_type(topo, type, obj))) {
		//pr_cpuset("get_cache_obj_by_pu", obj->cpuset);
		if (uti_hwloc_bitmap_isset(obj->cpuset, cpu->os_index)) {
			break;
		}
	}
	return obj;
...
uti_hwloc_obj_t get_numanode_obj_by_pu(uti_hwloc_obj_t cpu)
{
	uti_hwloc_obj_t obj = NULL;
	while ((obj = uti_hwloc_get_next_obj_by_type(topo, HWLOC_OBJ_NUMANODE, obj))) {
		if (uti_hwloc_bitmap_isset(cpu->nodeset, obj->os_index)) {
			break;
		}
	}
	return obj;
...
__attribute__((constructor)) void uti_init(void)
{
	...
	if ((ret = uti_hwloc_topology_init(&topo))) {
		pr_err("%s: error: uti_hwloc_topology_init\n",
			__func__);
		return;
	}

	if ((ret = uti_hwloc_topology_set_io_types_filter(topo, HWLOC_TYPE_FILTER_KEEP_ALL))) {
		pr_err("%s: error: uti_hwloc_topology_set_io_types_filter\n",
			__func__);
		return;
	}

	if ((ret = uti_hwloc_topology_load(topo))) {
		pr_err("%s: error: uti_hwloc_topology_load\n",
			__func__);
		return;
	}

	uti_hwloc_api_version = uti_hwloc_get_api_version();
	if (uti_hwloc_api_version < 0x00020000) {
		pr_err("%s: error: uti_hwloc_api_version (%x) is lower than expected. hwloc symbols might be resolved with unexpected object (e.g. MVAPICH).\n",
		       __func__, uti_hwloc_api_version);
		return;
	}
	
	if ((ret = uti_hwloc_topology_abi_check(topo))) {
		pr_err("%s: error: uti_hwloc_topology_abi_check: hwloc symbols might be resolved with unexpected object (e.g. MVAPICH).\n",
			__func__);
		return;
	}

	//pr_debug("%s: info: ncpus=%d\n", __func__, uti_hwloc_get_nbobjs_by_type(topo, HWLOC_OBJ_PU));

	while ((obj = uti_hwloc_get_next_obj_by_type(topo, HWLOC_OBJ_PU, obj))) {
	...
	shm_sz = uti_hwloc_get_nbobjs_by_type(topo, HWLOC_OBJ_PU) * sizeof(int);
...
{
	uti_hwloc_topology_destroy(topo);
...
static int uti_rr_allocate_cpu(uti_hwloc_cpuset_t cpuset)
{
	int i;
	int mincpu;
	unsigned int minrr;
...
static int uti_set_pthread_attr(pthread_attr_t *pthread_attr, uti_attr_t* uti_attr)
{
	...
	uti_hwloc_obj_t caller_cpu;
	uti_hwloc_cpuset_t cpuset = NULL, tmpset = NULL, env_cpuset = NULL;
	char *env_schedset_str;
	cap_t cap = NULL;

	/* Find caller cpu for later resolution of subgroups */
	caller_cpu_os_index = sched_getcpu();
	caller_cpu = uti_hwloc_get_pu_obj_by_os_index(topo, caller_cpu_os_index);
	if (caller_cpu) {
	...
	/* Initial cpuset */
	cpuset = uti_hwloc_bitmap_dup(uti_hwloc_topology_get_allowed_cpuset(topo));

	if (!(tmpset = uti_hwloc_bitmap_alloc())) {
                pr_err("%s: error: uti_hwloc_bitmap_alloc\n",
			__func__);
		ret = -ENOMEM;
		goto out;
	}
	...
	if (uti_attr->flags & UTI_FLAG_NUMA_SET) {
		uti_hwloc_bitmap_zero(tmpset);
		for_each_node(i, uti_attr->numa_set) {
			uti_hwloc_obj_t numanode = uti_hwloc_get_numanode_obj_by_os_index(topo, i);
			...
			if (numanode) {
				...
				uti_hwloc_bitmap_or(tmpset, tmpset, numanode->cpuset);
			}
		}
		uti_hwloc_bitmap_and(cpuset, cpuset, tmpset);
	}
	...
	if (uti_attr->flags &
		...
		uti_hwloc_obj_t caller_numanode =
		...
		if (caller_numanode) {
			if (uti_attr->flags & UTI_FLAG_SAME_NUMA_DOMAIN) {
				uti_hwloc_bitmap_and(cpuset, cpuset, caller_numanode->cpuset);
			}
			
			if (uti_attr->flags & UTI_FLAG_DIFFERENT_NUMA_DOMAIN) {
				uti_hwloc_bitmap_andnot(cpuset, cpuset, caller_numanode->cpuset);
			}
		}
	}
	...
	if (uti_attr->flags &
		...
		if (l1) {
			...
			if (uti_attr->flags & UTI_FLAG_SAME_L1) {
				uti_hwloc_bitmap_and(cpuset, cpuset, l1->cpuset);
			}
			
			if (uti_attr->flags & UTI_FLAG_DIFFERENT_L1) {
				uti_hwloc_bitmap_andnot(cpuset, cpuset, l1->cpuset);
			}
		} else {
	...
	if (uti_attr->flags &
		...
		if (l2) {
			...
			if ((uti_attr->flags & UTI_FLAG_SAME_L2) && l2) {
				uti_hwloc_bitmap_and(cpuset, cpuset, l2->cpuset);
			}
			
			if ((uti_attr->flags & UTI_FLAG_DIFFERENT_L2) && l2) {
				uti_hwloc_bitmap_andnot(cpuset, cpuset, l2->cpuset);
			}
		} else {
	...
	if (uti_attr->flags &
		...
		if (l3) {
			...
			if ((uti_attr->flags & UTI_FLAG_SAME_L3) && l3) {
				uti_hwloc_bitmap_and(cpuset, cpuset, l3->cpuset);
			}
			
			if ((uti_attr->flags & UTI_FLAG_DIFFERENT_L3) && l3) {
				uti_hwloc_bitmap_andnot(cpuset, cpuset, l3->cpuset);
			}
		} else {
	...
	if (!(env_cpuset = uti_hwloc_bitmap_alloc())) {
	...
	if (env_schedset_str) {
		...
		if (!(ret = string_to_glibc_sched_affinity(env_schedset_str, &env_schedset))) {
			uti_hwloc_cpuset_from_glibc_sched_affinity(topo, env_cpuset, &env_schedset, sizeof(cpu_set_t));
	...
	if (uti_attr->flags & UTI_FLAG_FABRIC_INTR_AFFINITY) {
		...
		for (uti_fabric = uti_fabrics; uti_fabric->name; uti_fabric++) { 
			uti_hwloc_obj_t fabric, package;

			pr_debug("uti_fabric->name: %s\n", uti_fabric->name);
			fabric = uti_hwloc_ibv_get_device_osdev_by_name(topo, uti_fabric->name);

			/* Find non-I/O ancestor which have cpuset */
			if (fabric) {
				package = uti_hwloc_get_non_io_ancestor_obj(topo, fabric);
				if (package) {
					pr_cpuset("package", package->cpuset);
					uti_hwloc_bitmap_and(cpuset, cpuset, package->cpuset);
				}
			}
		}
	}
	...
	if (uti_attr->flags &
		...
		uti_hwloc_bitmap_only(cpuset, uti_rr_allocate_cpu(cpuset));
	}

	/* Write cpuset to pthread_attr_t */
	ret = uti_hwloc_bitmap_weight(cpuset);
	if (ret > 0) {
		cpu_set_t schedset;
...
out:
	cap_free(cap);
	if (cpuset)
		uti_hwloc_bitmap_free(cpuset);
	if (tmpset)
		uti_hwloc_bitmap_free(tmpset);
	if (env_cpuset)
		uti_hwloc_bitmap_free(env_cpuset);
	return ret;
}
